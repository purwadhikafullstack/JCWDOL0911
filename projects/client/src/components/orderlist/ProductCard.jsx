      ...
      <img
        ...
        src={
          product.product_image
            ...
            : (product.prescription_image?prescriptionPic:medicine)
        }
        alt="gambar"
      />
      <div className="flex flex-col justify-between md:w-[70%] lg:[85%]">
        <div className="flex md:flex-row flex-col justify-between">
          <div className="flex flex-col justify-between">
            <div>{product.doctor?`Doctor: ${product.doctor}`:product.name}</div>
            <div>{product.patient ? `Patient: ${product.patient}` : null}</div>
            {product.type === "Bonus Item" ?
              <div>{`${product.quantity} + ${product.discount} (Bonus Item) = ${product.quantity+product.discount} `}{product.unit }</div>
            :
          ...
          {product.type === "Bonus Item" ? (
              ...
              <>
                <p color="black" className="text-sm font-medium">
                  {currency(
                    calculateDiscountedPrice(product.price, product.discount)*product.quantity
                  )}
                </p>
              </>
            ) : (
              <p color="black" className="text-sm font-medium">
                Price: {currency(product.prescription_price?product.prescription_price:product.price*product.quantity)}
              </p>
            )}
        </div>
