...
function AddressModal() {
  ...
  const renderCity = () => {
    return cities.map((city) => {
      return <option value={JSON.stringify(city)}>{city.type } { city.city_name}</option>
    })
  }
  const addAddress = () => {
    dispatch(addUserAddress(city,provinceId,street,isPrimary,district,postalCode,type))
    setOpen(false)
  }
  useEffect(() => {
    ...
    <div>
      ...
      >Add Address</button> :
        <div className='fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center '>
          ...
          <h5 class="text-xl font-medium text-gray-900 dark:text-white">Create New Address</h5>
          ...
          <select name="city" id="city" 
            ...
            <div class="flex items-center">
                <input id="link-checkbox" type="checkbox" value='true' class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                onChange={(   )=>setIsPrimary(!isPrimary)}/>
            <label for="link-checkbox" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Set As Primary</label>
            </div>
            <div className='flex gap-3 items-center justify-center py-3'>
              <button
               className='border-2 border-green-300  font-bold py-2 px-4 rounded-md hover:bg-emerald-500 hover:text-white text-emerald-500'
              onClick={()=>addAddress()}>Save</button>
              <button
              className='border-2 border-green-300  font-bold py-2 px-4 rounded-md hover:bg-emerald-500 hover:text-white text-emerald-500'
