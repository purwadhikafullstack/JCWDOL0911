...
function ManageAddressModal() {
    ...
    const addresses = useSelector(state => state.address.addressList.allAddress)
    const countAddress = useSelector(state => state.address.addressList.counData?.[0].count)
  ...
  return (
      <div>{!open ?
        ...
        <div className='fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center '>
              <div className=' bg-white py-3 lg:px-14 px-2 rounded-xl gap-2 flex flex-col shadow-lg w-96  lg:w-[32rem]'>
              <div className='flex justify-between items-end'>
                  <h5 class="text-xl lg:text-3xl pb-3 font-medium text-gray-900 dark:text-white">My Addres</h5>
          ...
          {offset + 3  >= pages ? <></> : <button type="button" class="bg-emerald-600 text-white rounded-r-md py-2 border-l border-gray-200 hover:bg-red-700 hover:text-white px-3"
            onClick={() => nextData()}>
            <div class="flex flex-row align-middle">
