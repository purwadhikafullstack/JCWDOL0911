...
function UserGraph({ chartSalesReport, dateRange }) {
  const dispatch = useDispatch();
  const { startDate, endDate } = dateRange;
  const salesReport = useSelector((state) => state.report.salesReport);
  const [userData, setUserData] = useState({
    labels: chartSalesReport.map(
      (report) => report.full_name || report.username
    ),
    datasets: [
      {
        label: "User Spending Mwahahaha",
        data: chartSalesReport.map((report) => report.TotalPrice),
      },
    ],
  });
  ...
  const onChangeSortingHandler = (e) => {
    if (e.target.value === "asc") {
      dispatch(ascUserReport());
    } else if (e.target.value === "desc") {
      dispatch(descUserReport());
    }
  };
  ...
  return (
    <div>
      ...
      <div className="w-full text-center mt-4">Customer Name</div>
      <hr className="my-4" />
          ...
          return (
            ...
            >
              ...
              <div className="flex sm:flex-row flex-col sm:gap-10 gap-2">
                ...
                <div>Spending : {currency(report.TotalPrice)}</div>
              </div>
            </div>
          );
        })}
