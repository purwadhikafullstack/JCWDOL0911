...
export const { setPrimary,setAddressList,setProvinces,setCount } = addressSlice.actions
...
export function fetchProvince() {
    return async (dispatch) => {
      let response = await axios.get(`${process.env.REACT_APP_API_BE}/addresses/provinces`);
      dispatch(setProvinces(response.data));
    }
  }
  export function addUserAddress(city,idProvince,street,isPrimary,district,postal_code,type) {
      ...
      const offset = 0
    return async (dispatch) => {
      let response = await axios.post(`${process.env.REACT_APP_API_BE}/users/address`, { idUser, idCity, idProvince, street, isPrimary, district, postal_code,cityName,type });
      Swal.fire(
        `${response.data.message}`,
        '',
        'success'
